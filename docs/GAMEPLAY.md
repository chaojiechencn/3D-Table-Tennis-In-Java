...
The racket follows your mouse, and only your mouse. It slides around on one flat plane at bat
height: move the mouse **across** and the racket goes across, move it **up the screen** and the
racket moves up the table toward the net, **down** and it comes back behind the baseline for a
...
to open the face and cut backspin under it. A still racket just blocks it back soft. **Hold the
right mouse button** and the up/down axis stops moving the racket up the table and starts raising
...
What that buys, measured over 75 different ways of swinging the racket at the same ball: the raw
...
- Mouse-controlled racket that slides anywhere from over the table to well behind the baseline,
...
- Multiple rackets that play differently — one built for spin, one for power
...
| Input | Action |
| --- | --- |
| Mouse left / right | Moves the racket across the table |
| Mouse up / down | Moves the racket up the table toward the net, or back behind the baseline |
...
| Swipe the racket sideways | Sends the ball that way — swipe right, ball goes right |
| Drive the racket up-table through the ball | Pace, depth and topspin |
| Pull the racket back through the ball | Opens the face and cuts backspin |
| Still racket | Soft block back |

There is no hit button — the shot is entirely in the mouse movement. Normally the racket stays at
...
| `V` | Shot-assist debug overlay (racket and ball velocity, raw / intended / final shot, target, predicted landing) |
| `D` | Control debug overlay (cursor, racket and target position, legal racket area, how far and how long the racket has to travel, where the ball is and when it arrives, and whether you could have got there) |
