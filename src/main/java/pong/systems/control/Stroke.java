...
/**
 * The player's racket: follows the mouse and nothing else. {@code advance} takes no
 * {@code Ball} -- the ball is never an input, enforced by the signature rather than by
 ...
 * {@code pong._tests.RallyTest}. See docs/DESIGN.md for why the old ball-driven reach and the
...
public final class Stroke {
     ...
     * into spin. TUNED: 0.55 puts a full-speed drive at the face tilt PhysicsTest measures a real
     ...
     *
     * Racket derives angular velocity from how far the normal turned in one step, so a snapped
     ...
     *
     * @param dt the PHYSICS step, never a frame time -- Racket derives its velocity from this,
    ...
    public void advance(Racket blade, double dt) {
        ...
        if (moved.length() > STROKE_EPS * dt) {
            ...
            // "the racket's face settling to square", for the 57-degree bug this replaced.
