 ...
 *
 * This exists so there can still be exactly ONE impulse solver once a racket joins the table,
 ...
 *
 * The fourth question is the one that matters for a racket. A table does not move, so the
 * three original surfaces answer {@link Vec3#ZERO} and nothing changes for them. A racket
 ...
 *
 * The two implementations are {@link Contacts.Box} (table, net, floor) and
 * {@code Racket.Blade}. This was a {@code sealed} interface while they shared a package; they
 * no longer do, and an unnamed module cannot seal across packages. Nothing switched on the
 * type anyway -- every caller goes through the four questions below -- so the seal was
 * documentation rather than enforcement, and this comment now carries it.
 */
