package pong.systems.shotmaking;

import static pong.config.Physical.BALL_R;
import static pong.config.Physical.BLADE_R;
import static pong.config.Physical.DT;
import static pong.config.Physical.NET_HEIGHT;
import static pong.config.Physical.TABLE_LENGTH;
import static pong.config.Physical.TABLE_WIDTH;
import pong.config.ShotTuning;
...
public final class ShotAssist {
    ...
    public ShotAssist()                { this(new ShotTuning()); }
    public ShotAssist(ShotTuning tune) { this.t = tune; }
     ...
     */
    public Ball assist(Ball incoming, Ball physical, Racket racket, boolean playerHit) {
        ...
        return new Ball(physical.pos(), finalVel, finalSpin, physical.orient());
    }
    ...
    private Intent readIntent(Vec3 contact, Racket racket, double toOpp) {
        Vec3 swing = racket.vel();
     ...
     */
    private double quality(Ball incoming, double offX, double offY) {
    ...
    private double readSpeed(double swingAmount, Ball incoming) {
    ...
    private static Flight fly(Vec3 from, Vec3 vel, Vec3 spin, double toOpp) {
        Ball s = Ball.at(from, vel, spin);
        double netHeight = Double.NaN;
        double prevZ = from.z();

        for (int i = 0; i < (int) (3.0 / VALIDATE_DT); i++) {
            Ball next = Integrator.step(s, VALIDATE_DT);
            Vec3 p = next.pos();
