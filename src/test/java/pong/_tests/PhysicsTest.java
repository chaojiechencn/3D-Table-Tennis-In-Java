...
import pong.game_objects.ball.Aero;
...
public final class PhysicsTest {
    ...
    private static void terminalVelocityMatchesClosedForm() {
        ...
        Ball s = Ball.at(new Vec3(0, 500, 0), Vec3.ZERO, Vec3.ZERO);
    ...
    private static void verticalDropMatchesAnalyticSolution() {
        ...
        Ball s = Ball.at(new Vec3(0, y0, 0), Vec3.ZERO, Vec3.ZERO);
    ...
    private static void ittfDropTest() {
        World w = new World();
        w.launch(Ball.at(new Vec3(0, 0.305 + BALL_R, -0.7), Vec3.ZERO, Vec3.ZERO));
    ...
    private static double reboundWithRestitution(double e) {
        Ball s = Ball.at(new Vec3(0, 0.305 + BALL_R, -0.7), Vec3.ZERO, Vec3.ZERO);
    ...
    private static void rk4IsFourthOrder() {
        Ball start = Shots.byName("Topspin loop").state();   // drag and Magnus both active
        double t = 0.4;
    ...
    private static Vec3 integrateFor(Ball s, double seconds, double dt) {
    ...
    private static void magnusCurvesTheRightWay() {
        ...
        double flat = firstLandingZ(Ball.at(pos, vel, Vec3.ZERO));
        double top  = firstLandingZ(Ball.at(pos, vel, new Vec3(-heavy, 0, 0)));
        double back = firstLandingZ(Ball.at(pos, vel, new Vec3(heavy, 0, 0)));
    ...
    private static void sidespinDeflectsTheRightWay() {
        ...
        double left  = firstLandingX(Ball.at(pos, vel, new Vec3(0, spin, 0)));
        double right = firstLandingX(Ball.at(pos, vel, new Vec3(0, -spin, 0)));
    ...
    private static void topspinKicksForwardBackspinChecks() {
        ...
        double[] top  = bounceChange(Ball.at(pos, vel, new Vec3(-spin, 0, 0)));
        double[] back = bounceChange(Ball.at(pos, vel, new Vec3(spin, 0, 0)));
    ...
    private static double[] bounceChange(Ball start) {
        World w = new World();
        w.launch(start);
        Ball before = start;
        for (int i = 0; i < 480 * 3; i++) {
    ...
    private static void outOfBoundsIsDetected() {
        ...
        w.launch(Ball.at(new Vec3(0, 0.35, 1.5), new Vec3(-6, 1.0, -9), Vec3.ZERO));
    ...
    private static boolean landsOut(Ball s) {
        World w = new World();
        w.launch(s);
    ...
    private static void longRunStaysStable() {
        ...
        // by deleting the floor rather than by pong. What it is really testing is that the
    ...
    private static void noTunnellingAtSmashSpeed() {
        ...
        for (double speed = 20; speed <= 60; speed += 2.5) {
            tested++;
            World w = new World();
            w.launch(Ball.at(new Vec3(0, 0.30, -0.5), new Vec3(0, -speed, 0), Vec3.ZERO));
            for (int i = 0; i < 480; i++) {
                w.step();
    ...
    private static double firstLandingZ(Ball s) { return Aim.landingPoint(s).z(); }
    private static double firstLandingX(Ball s) { return Aim.landingPoint(s).x(); }
    ...
    // racket is doing physics rather than running a scripted shot.
    //
    ...
    private static Ball racketStrike(Ball ball, Vec3 endPos, Vec3 swing,
        ...
        Racket.Blade blade = racket.collider();
     ...
     * speed. This is the most basic thing a racket has to do, and the old solver could not do
    ...
    private static void swingSpeedSplitsIntoPaceAndSpin() {
        double swing = 17.8;
        Ball arriving = Ball.at(new Vec3(0, 0.30, 0), new Vec3(0, 0, 10), Vec3.ZERO);

        Ball loop = brush(arriving, swing, 30);   // 30 degrees up: a looping stroke
        Ball drive = brush(arriving, swing, 0);   // straight through: a drive
    ...
    private static void brushingContactGeneratesTopspin() {
        ...
        // Ball now heading toward -Z, so topspin is rotation about -X (Ball's convention).
    ...
    private static void racketReversesIncomingBackspin() {
        ...
        Ball chop = Ball.at(new Vec3(0, 0.30, 0), new Vec3(0, 0, 5), backspin);
    ...
    /**
     * A racket is allowed to add energy -- that is what a swing is for -- but only as much as
    ...
    /**
     * The racket equivalent of the table tunnelling check, and a harder problem than the table
    ...
    private static void noTunnellingThroughASwungRacket() {
        ...
        for (double speed = 20; speed <= 60.01; speed += 2.5) {
            tried++;
            Ball ball = Ball.at(new Vec3(0, 0.30, 0), new Vec3(0, 0, speed), Vec3.ZERO);
            Ball flown = Integrator.step(ball, DT);
            ...
            Racket racket = new Racket(new Vec3(0, 0.30, meet + blade * DT), new Vec3(0, 0, -1));
            racket.moveTo(new Vec3(0, 0.30, meet), new Vec3(0, 0, -1), DT);

            if (Contacts.detect(ball, flown, racket.collider()) != null) caught++;
        }
        check("no tunnelling through a swung racket from 20 to 60 m/s",
              caught == tried,
