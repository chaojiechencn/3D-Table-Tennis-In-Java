 ...
 *
 * The second group covers the player's racket, which has the same problem from the other side:
...
public final class RallyTest {
    ...
    private static Rally feed(Shots shot) {
        ...
        Racket blade = new Racket(new Vec3(0, 0.20, Follower.PLANE_Z), new Vec3(0, 0, 1));
        Opponent ai = new Follower();
        ShotAssist assist = new ShotAssist();

        world.setRackets(null, blade);
     ...
     * it. PhysicsTest was right not to -- no contact ever added energy, the blade simply kept
    ...
    private static Return playOut(Shots shot) {
        ...
        for (int i = 0; i < (int) (6.0 / DT); i++) {
            ai.advance(world.state(), blade, DT);
            Ball prev = world.state();
            double before = prev.pos().y();
            world.step();

            if (!hit && world.racketHits() > 0) {
                hit = true;
                hitAt = i;
     ...
     * hands the blade a whole frame of travel to cover inside a single 1/480 s step. Racket
     ...
     * control path gains any knowledge of the ball. Stroke still has no Ball parameter, so
    ...
    private static boolean playThePoint(Shots shot) {
        ...
        Racket me = new Racket(PlayerReach.NEUTRAL, new Vec3(0, 0, -1));
    ...
    private static void theBrushLiftsTheBatWithoutExtendingItsReach() {
        ...
        check("the brush cannot cut the bat down through the table top",
              lowest >= pong.config.Physical.BLADE_R - 1e-9,
              String.format("lowest blade centre %.3f m against a blade radius of %.3f",
                            lowest, pong.config.Physical.BLADE_R));
